...
(define (find-blank puz )
  (find-blank-helper puz 0 0))

(define (find-blank-helper puz x y)
  (cond 
    [(empty? (puzzle-constraints puz)) false]
    [(equal? (first (first (puzzle-constraints puz)))
             (first (first (puzzle-board puz)))) (make-posn x y)]
    [(and (guess? (first (first (puzzle-board puz))))
          (equal? (guess-symbol (first (first (puzzle-board puz))))
                                (first (first (puzzle-constraints puz))))) 'guess]
    [else (find-blank (make-puzzle
                       (puzzle-size puz)
                       (puzzle-board puz)
                       (rest (puzzle-constraints puz))) (add1 x) (add1 y))]))
;; Tests
...
(define (used-in-row puz posn1)
  (quicksort (map 
             ...
             (filter (lambda (x) (cond 
                               ...
                               [(number? x) true]
                               [else false]))
                    (list-ref (puzzle-board puz) (posn-y posn1)))) <))
...
;;Function Definition 
 (define (used-in-column puz pos)
   (quicksort 
     (map (lambda (z) (cond 
                        [(guess? z) (guess-number z)]
                        [else z]))
     (filter (lambda (y) (cond 
                           [(number? y) true]
                           [(guess? y) true]
                           [else false])) 
...
(define (available-vals puz pos)
  (quicksort
     (filter (lambda (x) (not (zero? x)))
           (map (lambda (y) (cond 
                              [(not (member? y (append (used-in-column puz pos)
                                                       (used-in-row puz pos)))) y]
                              [else 0])) 
...
(define (place-guess brd pos val)
    (filter (lambda (x) (not (empty? x))) (place-guess-1 brd pos val)))

(define (place-guess-1 brd pos val)
  (cond 
    [(empty? brd) empty]
    [(equal? 0 (posn-y pos)) (cons (replace-col (first brd) pos val) (rest brd))]
    [else  (cons (first brd) (place-guess (rest brd)
                                        (make-posn (posn-x pos)
                                                   (sub1 (posn-y pos)))
                                        val))]))
(define (replace-col lst pos val)
  (cond 
    [(empty? lst) empty]
    [(equal? 0 (posn-x pos)) (cons (make-guess (first lst) val) (rest lst))]
    [else (cons (first lst) (replace-col (rest lst) 
                                         (make-posn 
                                           (sub1 (posn-x pos))
                                           (posn-y pos))  val))]))
...
(check-expect (fill-in-guess puzzle2soln (make-posn 5 4) 3) (make-puzzle
                                                             6
                                                             (list
                                                              (list 5 6 3 4 1 2)
                                                              (list 6 1 4 5 2 3)
